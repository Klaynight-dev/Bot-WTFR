...
export async function execute(interaction: ChatInputCommandInteraction) {
  ...
  if (minutes <= 0 || minutes > maxMinutes) {
    return interaction.reply({ content: `Durée invalide — entre 1 et ${maxMinutes} minutes (28 jours).`, flags: MessageFlags.Ephemeral })
  }

  // verification permissions et hiérarchie
  const botMember = interaction.guild.members.me
  if (!botMember || !botMember.permissions.has?.('ModerateMembers')) {
    return interaction.reply({ content: "Le bot n'a pas la permission \"MODERATE_MEMBERS\".", flags: MessageFlags.Ephemeral })
  }

  // cannot moderate guild owner
  if (member.id === interaction.guild.ownerId) {
    return interaction.reply({ content: "Impossible de mute le propriétaire du serveur.", flags: MessageFlags.Ephemeral })
  }

  // role hierarchy: bot must be higher than target
  if ((botMember.roles?.highest?.position ?? 0) <= (member.roles?.highest?.position ?? 0)) {
    return interaction.reply({ content: "Impossible : le rôle du membre est égal ou supérieur à celui du bot.", flags: MessageFlags.Ephemeral })
  }
  ...
  try {
    ...
    if ((interaction.guild.ownerId !== (interaction.user.id)) && invokerPos <= targetPos) {
      return interaction.reply({ content: "Tu ne peux pas mute un membre avec un rôle égal ou supérieur au tien.", flags: MessageFlags.Ephemeral })
    }
  } catch (_) {
  ...
  try {
    ...
    await interaction.reply({ content: `✅ ${user.tag} mis en timeout (${minutes} min).`, flags: MessageFlags.Ephemeral })
  } catch (err: any) {
    console.error('mute error:', err)
    const apiMsg = err?.message || String(err)
    await interaction.reply({ content: `❌ Impossible de placer en timeout. Erreur API: ${apiMsg}`, flags: MessageFlags.Ephemeral })
  }
}
