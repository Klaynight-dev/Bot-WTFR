...
export async function execute(interaction: ChatInputCommandInteraction, client: any) {
  ...
  try {
    ...
    await replyEphemeralEmbed(interaction, createSuccessEmbed('Message public envoyé / mis à jour.'))
  } catch (err) {
    console.error(err)
    try { await replyEphemeralEmbed(interaction, createErrorEmbed('Échec lors de l’envoi du message public.')) } catch (_) { }
  }
} 
